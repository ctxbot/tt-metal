...
# ------------------------------------------------------------------------------
# Setup for this test (fabric2d-mesh-4x2-1link):
#   dispatch_group_size                = 4  (chips along SP axis)
...
#   max_dispatched_tokens_per_expert   = 4 * 256 = 1024
#   raw dispatch-buffer capacity       = 1024 * 1 = 1024  (factor=1)
#   max_dispatch_buffer_token_size     = 1024 + 32 = 1056  (one tile of alignment reserve)
#
# Indices tensor shape: (dispatch_group_size, seq_len_per_chip, num_experts_per_tok) = (4, 256, 2)
...
#   indices[..., 0] = 14
#   indices[..., 1] = 15
#   Every (chip, token) picks (14, 15). Counts: expert 14 gets 1024 tokens,
#   expert 15 gets 1024 tokens, both targeting chip 7's flat buffer.
#   Chip 7's buffer (1056 slots) fills slots 0..1023 with expert 14. The alignment
#   reserve admits 32 tokens from expert 15 and drops its remaining 992 tokens.
#
# Case "cut_short_last":
...
#   Counts: expert 14 gets 4*153 = 612, expert 15 gets 4*153 = 612
#           (both on chip 7); experts 0 and 1 get 4*103 = 412 each (on chip 0).
#   Chip 7's buffer: expert 14 writes 612 tokens, then expert 15 starts at the
#   tile-aligned offset 640. Only 1056-640 = 416 slots remain, so 196 of expert
#   15's tokens are dropped mid-expert.
#   Chip 0's aligned regions end at 416 + 412 = 828 < 1056, so it does not overflow.
#
