...
//
// Built on the Metal 2.0 ProgramSpec API rather than the CreateCircularBuffer + QuasarDataMovementConfig
// path the rest of this file uses. That is deliberate: the legacy Quasar shim has no way to disable
// implicit DFB sync (grep implicit_sync in temp_quasar_api.hpp finds nothing), and per #45160 implicit
// sync is default-enabled on a Gen2 DM kernel. A reader that also does its own reserve_back/push_back
// therefore gets double-synced and deadlocks. ProgramSpec exposes disable_dfb_implicit_sync_for_all,
// which every passing Quasar compute test in this repo sets.
struct MatmulNoMopConfig {
    uint32_t rt_dim = 1;
    uint32_t ct_dim = 1;
    uint32_t kt_dim = 1;
...
void run_matmul_no_mop(
    const std::shared_ptr<distributed::MeshDevice>& mesh_device, const MatmulNoMopConfig& test_config) {
    auto& cq = mesh_device->mesh_command_queue();
    auto zero_coord = distributed::MeshCoordinate(0, 0);
    auto device_range = distributed::MeshCoordinateRange(zero_coord, zero_coord);
    distributed::MeshWorkload workload;
    ...
    // Quasar drives dataflow through the Gen2 config and takes its DFB sync explicitly; Gen1 arches
    // pin the processor/NOC pair per direction.
    ...
    std::vector<float> golden_floats = make_matmul_golden(stimulus.in0_floats, stimulus.in1_floats, rt_dim, kt_dim, ct_dim);
...
// Compute-layer coverage for the experimental MOP-less matmul (issue #52329). Ordered so the first
// failure localises: 1x1x1 is a single tile through init + execute, then kt_dim>1 adds dest
// accumulation across inner steps, then rt/ct > 1 adds the block reuse walk.
TEST_F(LLKQuasarMeshDeviceSingleCardFixture, TensixTestSingleCoreComputeMatmulNoMop) {
    using unit_tests::compute::matmul::MatmulNoMopConfig;
    const std::vector<MatmulNoMopConfig> cases = {
