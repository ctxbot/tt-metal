...
// emit_sem_check_bindings: the USAGE-CHECK variant of the sem-ids section (see JitBuildSettings::
// is_sem_usage_check). The scope table is emitted separately as the header's first line exactly
// like a real build (placeholder scopes -- the check runs BEFORE refinement and feeds it); here
// each sem:: id becomes a tag TYPE whose uint32_t conversion is deprecation-poisoned, so
// Semaphore's check-mode Tag parameter (noc_semaphore.h, TT_SEM_USAGE_CHECK) makes every method
// diagnostic name its semaphore. No tripwires: the check TU is frontend-only and never runs.
inline void emit_sem_check_bindings(std::ostream& os, const std::vector<SemBindingEntry>& entries) {
    os << "#include <cstdint>\n";
    os << "#ifndef COMPILE_FOR_TRISC\n";
    os << "#include \"api/dataflow/noc_semaphore.h\"\n";
    os << "#endif\n";
...
public:
    ...
    // True only for the census-time semaphore USAGE CHECK (program_spec.cpp RunSemUsageCheck):
    // the bindings header then emits sem:: ids as poisoned tag TYPES (emit_sem_check_bindings)
    // instead of plain uint32_t, so a -fsyntax-only compile reports every semaphore op.
    virtual bool is_sem_usage_check() const { return false; }
