...
/*************************************************************************
 * LLK MATMUL NO MOP (Quasar)
 *************************************************************************/

// Whether the operand pair drives the MXFP4_2x matmul variant. Mirrors llk_math_matmul_init: in0 lands
// in SrcB and in1 in SrcA, and the two must agree on 2x-ness. Re-derived per call rather than cached at
// init so the execute path stays stateless (the MOP path keeps this implicitly in its MOP config, which
// the no-MOP path has by definition given up).
inline bool matmul_no_mop_is_2x_format(const std::uint32_t operandA, const std::uint32_t operandB) {
    ...
    return is_2x_format(srcA_format) && is_2x_format(srcB_format);
}
 ...
 * @tparam THROTTLE_LEVEL: Accepted for API parity with Wormhole/Blackhole; Quasar has no throttled MVMUL sequences, so only 0 is valid
...
inline void llk_math_matmul_init_no_mop(
    ...
    static_assert(
        THROTTLE_LEVEL == 0, "Quasar no-mop matmul only supports THROTTLE_LEVEL == 0; Quasar has no throttled MVMUL sequences");
    ...
    if (matmul_no_mop_is_2x_format(operandA, operandB)) {
