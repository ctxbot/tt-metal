...
// Unified cores, shared by the CB-id API below and the L1MetaData API (experimental/). They take
// already-resolved scalar formats/geometry + the runtime write address; the per-source prologue
// (resolving these from a CB id, or from an L1Descriptor) lives in the callers.
template <
