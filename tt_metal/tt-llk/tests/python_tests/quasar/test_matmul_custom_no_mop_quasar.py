...
THROTTLE_LEVEL == 0 (same posture as Wormhole).
"""
...
def _run_matmul_custom_no_mop(
    ...
    # MX inputs are quantized onto their lattice by the unpacker, so the golden has to see the
...
@parametrize(
    ...
    # dimensions depends on (formats, dest_acc): the max result-tile count is set by which dest
    # register the pair lands in (see _dest_bank_max_tiles).
    dimensions=runtime(
...
MATMUL_2X_FORMATS = [
    InputOutputFormat(
        DataFormat.MxFp4, DataFormat.Float16_b, register_format_hint=hint
    )
