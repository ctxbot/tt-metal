...
/*************************************************************************
 * LLK MATH MATMUL CUSTOM NO MOP - matmul without a MOP (Quasar)
 *************************************************************************/
...
//
// Scope: full 32x32 tiles, matching the regular Quasar matmul (llk_unpack_AB_matmul_api.h indexes L1 in
// whole tiles, tt-metal #45208). The direct-indexing (MVMULDI) variant is deliberately not covered - no
// Compute API surface selects it, for the MOP path either.
...
inline void _llk_math_matmul_run_no_mop_(const bool reuse_a)
{
    ...
    for (std::uint32_t phase = 0; phase < FIDELITY_PHASES - 1; phase++)
    {
        ...
        // matmul_op: close this fidelity phase, rewind dest to the start of the tile, advance the
        // fidelity counter.
