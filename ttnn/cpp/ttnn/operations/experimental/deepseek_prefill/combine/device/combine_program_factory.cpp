...
constexpr uint32_t semaphores_per_fabric_connection = 2;
constexpr uint32_t l1_semaphore_slots_per_core = 16;
...
tt::tt_metal::ProgramDescriptor build_program_for_coord(
    ...
    for (const auto& sender_core : sender_cores) {
        ...
        if (num_links > 0) {
            ...
            if (is_2d_fabric) {
                TT_FATAL(
                    dst_nodes.size() <= max_fabric2d_sender_connections,
                    "FABRIC_2D combine needs {} physical first-hop connections from {}, but the device manager "
                    "supports at most {}",
                    dst_nodes.size(),
                    src_fabric_node_id,
                    max_fabric2d_sender_connections);
                const uint32_t occupied_sender_semaphores = per_core_sema_slots[sender_core].count();
                const uint32_t required_sender_semaphores =
                    occupied_sender_semaphores + semaphores_per_fabric_connection * dst_nodes.size();
                TT_FATAL(
                    required_sender_semaphores <= l1_semaphore_slots_per_core,
                    "FABRIC_2D combine needs {} sender-core semaphores ({} already occupied + {} connections x "
                    "{}), but only {} slots are available",
                    required_sender_semaphores,
                    occupied_sender_semaphores,
                    dst_nodes.size(),
                    semaphores_per_fabric_connection,
                    l1_semaphore_slots_per_core);
            }
