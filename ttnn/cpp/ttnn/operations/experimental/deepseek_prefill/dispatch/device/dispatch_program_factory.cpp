        ...
        if (operation_attributes.num_links > 0) {
            ...
            if (is_2d_fabric) {
                TT_FATAL(
                    ...
                    "FABRIC_2D dispatch needs {} physical first-hop connections from {}, but the device manager "
                    "supports at most {}",
                    dst_nodes.size(),
                    src_fabric_node_id,
                    max_fabric2d_sender_connections);
                const uint32_t required_sender_semaphores =
                    num_workers + semaphores_per_fabric_connection * dst_nodes.size();
                TT_FATAL(
                    required_sender_semaphores <= l1_semaphore_slots_per_core,
                    "FABRIC_2D dispatch needs {} sender-core semaphores ({} workers + {} connections x {}), but "
